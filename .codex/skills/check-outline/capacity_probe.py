#!/usr/bin/env python3
"""check-outline 的容量探針:直接讀沙箱 validator + 模板包 manifest 的合併容量。
...
用法(repo 根目錄執行,單行、相對路徑、三種 shell 通用):
  python .codex/skills/check-outline/capacity_probe.py --list-caps cover
  python .codex/skills/check-outline/capacity_probe.py --check cover main_title "候選主標題"
  python .codex/skills/check-outline/capacity_probe.py --check agenda items 7
  python .codex/skills/check-outline/capacity_probe.py --check data_three_number_kpis "kpis[].value" "12%"
...
def main():
    ...
    ap.add_argument("--sandbox", default="ppt_out")
    ...
    ctx = load_merged(Path(args.sandbox), args.template_pack)
    ...
    if pt not in merged:
        ...
        die(f"[E] 未註冊頁型 {pt!r}(全集跑 python {args.sandbox}/tools/make_skeleton.py --list)")
