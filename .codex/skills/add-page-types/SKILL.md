...
`engine/templates/<id>/bindings.json` 的 `fills` 加一筆
`{"template_page": N, "ops": [...]}`。
...
- 清單要配 `delete_on_missing`(項目少於格位時刪多餘格);格位不夠時配
  `overflow.merge_into_id`。
- 例外:清除窗內的純數字頁碼框(x>11.2 且 y>6.3)由引擎處理,不用管。

**注意平行欄位的格位數要一樣**。light p17 三欄分別有 6/6/4 個格位,
上限開到 5 就會讓第三欄併格、三欄長得不一樣(設計師目檢 p4 抓到)。
取最少的那個當上限。
...
把結果寫進 manifest 的 `capacity_overrides`。想看提案不寫入用 `--dry-run`;
契約大改過就加 `--reset` 從零重量。

演算法與 **9 個已知量測陷阱**寫在 `engine/release/fit_capacity.py` 檔頭
(清單路徑的 `.item`、切片位移、併格框的長度公式、一框多欄位、
`add_textbox` 的跨槽位預算、探測字元要用中文、群組座標換算、
碰撞基準取自模板、只收「範圍會隨文字變大」的那一方)。
**要改那支之前先讀完檔頭**,這些坑每一個都真的踩過。
...
同步更新:`page_map.md`(clone→fill)、`page_types.md` 該節補
「已註冊頁型」標記句、registry 的容量表與頁型總數、instructions 的
版本字串與頁型數、manifest `version` 進版、`engine/REGRESSION.md` 的
R7 sha 基準。最後跑一次 `lint --all` + `golden` 確認沒改壞。
...
| lint:inventory 缺第 N 頁快照 | 先跑 `freeze --id <id>` |
