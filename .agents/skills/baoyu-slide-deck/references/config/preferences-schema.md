...
| `preferred_image_backend` | string | `imagegen` | Locked to Codex's native `imagegen` by policy (no non-Codex tools). Resolution logic is documented in `SKILL.md`'s `## Image Generation Tools` section; if `imagegen` is unavailable the skill reports a blocker rather than substituting another backend. |
