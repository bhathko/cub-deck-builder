...
**⛔ This install is locked to Codex (company policy).** Image generation MUST use Codex's own `imagegen` tool and nothing else. Do NOT use Cursor `GenerateImage`, `baoyu-image-gen`, `codex exec` wrappers, or any other non-Codex backend — even if one happens to be available.

Resolve the backend as follows:

1. **Codex (`imagegen`)** — inspect your available-skills / tool inventory. If a skill named `imagegen` is listed, you are running inside Codex: invoke it via the `Skill` tool with `skill: "imagegen"`, passing the saved prompt file's content (plus output path and aspect ratio per Codex `imagegen`'s own args). This is the only approved raster backend.
2. **If `imagegen` is not listed** — STOP and report a blocker: tell the user Codex's `imagegen` tool is not exposed in this runtime and ask them to enable it. Do NOT fall back to any other tool, and do NOT silently downgrade to SVG/HTML/canvas/Pillow.
...
The image backend is locked to Codex `imagegen` by policy; any `preferred_image_backend` override to a non-Codex tool is ignored.
...
The only approved image tool is Codex `imagegen`.
...
`{baseDir}` = this SKILL.md's directory. The merge scripts run on **Node.js (v18+)** — no bun required. First use only: run `npm install` inside `{baseDir}/scripts/` once to fetch `pptxgenjs` and `pdf-lib`; afterwards invoke with `node`.

| Script | Purpose |
|--------|---------|
| `scripts/merge-to-pptx.mjs` | Merge slides into PowerPoint |
| `scripts/merge-to-pdf.mjs` | Merge slides into PDF |
...
At generation time, verify files exist. If `usage: direct` and Codex `imagegen` accepts reference images, pass the file on every slide. Otherwise embed extracted `style`/`palette` traits in the prompt text.
...
1. Resolve the image backend via the Image Generation Tools rule at the top — Codex `imagegen` only; if it is not available, report a blocker instead of substituting another tool.
...
```bash
# First run only: install merge deps (one time)
npm --prefix {baseDir}/scripts install
# Merge (Node.js, no bun)
node {baseDir}/scripts/merge-to-pptx.mjs <slide-deck-dir>
node {baseDir}/scripts/merge-to-pdf.mjs <slide-deck-dir>
```
...
- **Common one-line edits**:
  - `preferred_image_backend: imagegen` — Codex's built-in image tool (the only approved backend; locked by policy).
