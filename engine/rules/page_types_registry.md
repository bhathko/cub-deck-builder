...
1. **已註冊頁型(本檔下列各節)**:驗證器有完整槽位契約——槽位名稱、數量與
   字數上限必須完全符合,否則以 ERROR 擋下、不會產檔。字數上限以「字元數」計
   (含標點與空白)。**這些頁型由 render_deck.py 全自動產出**(clone 模板實頁後
   依綁定填字),版面不經過任何 AI 判斷,同一份 JSON 產一萬次結果相同。
   (本檔刻意不寫「共 N 種」——頁型會隨模板加開而增加,寫死的數量一旦過期就會
   讓人/模型誤以為新頁型不能用。當下全集跑 `make_skeleton.py --list`。)
2. **頁型庫其他頁型(知識庫 `page_types.md` 裡尚未註冊的那些)**:允許使用,`page_type` 填
