...
手寫的跨模板預設、表是 light 實測,fit 一跑就差更多);最糟的是三大數字 KPI
散文說「value ≤6 字、可含單位、如 99.9%」而實際上限 2 字——舉的例子全放不進去。
修法不是把數字改對(下次 fit 又錯),是**散文不再寫數字**,容量表改為**全量
312 列**成為唯一數字真相。②`page_types.md` 檔頭講死分工(語意在此、數量看工具),
並修正三層金字塔「左右共 4-6 組」(昨天已收成固定 6 組)。③三大數字 KPI 的
value 版位 1.76/1.43/1.33 吋 @66pt 統一置中放寬到 2.90 吋 → 上限 2→5 字,
「99.9%」「1,200」放得進去了(該頁型先前實質不可用,實測產檔時撞到)。
**不加 regex 閘門守文件數字**——add-page-types 已記過教訓:實測 1 真命中 :
2 假警報 : 1 漏報,本次掃描也再現假警報(「6-8 張卡片」是跨列總數)。)
