...
> **用途**:記錄這個專案從無到有的決策過程、每次方向修正的原因、已知風險與
> 未驗證項。**這裡的「為什麼」在別處找不到**——程式碼只告訴你「是什麼」。
> **讀者**:接手的 agent / 維護者。
> **何時讀**:改任何東西之前(至少掃過章節標題);想知道「為什麼不那樣做」時。
>
> 讀法:§0–§7 是 GPTs 版的起源與早期架構,§8 是待辦與升級計畫,
> §9–§19 是逐版演進,§20–§22 是多模板架構四個 Phase,§21 附 repo 重構的
> 新舊路徑對照。**歷史章節的路徑一律保留當時狀態**(如 `gpts/tools/`),
> 不隨重構改寫——要對照現況查 §21。
