...
| 為什麼**沒有** builtin 頁型了 | 結論與理由都被推翻:builtin 不是手工雕刻而是描模板頁,所以能整批遷成 fill;2026-07-26 清零、07-27 連分派路徑與所有現況文字裡的字樣一起刪 | [10.5](#105-builtin-清零2026-07-26含一個推翻先例的決定) / [10.6](#106-builtin-死碼清除2026-07-27把無人使用變成不存在) / [10.7](#107-builtin-字樣清零2026-07-27現況文字與歷史紀錄分家) |
...
- ~~**新模板不得有 builtin 頁型**(builtin 只保留給 light 作 grandfather)。~~
  **⚠️ 這條已被 §10.5/§10.6 推翻並超越:builtin 不只「新模板不得有」,
  而是整個模式從引擎拆除,light 也沒有。現行規則——任何包的綁定都只能是
  `bindings.json`,支援等級只有 fill/clone/unsupported。**
  當年的理由仍成立且被沿用:座標寫死的手工雕刻(cover 的寬度估算、
  agenda 的 spacing 公式)錯誤面大且無法被 lint 覆蓋;fill 模式與設計師的
  工作方式一致(在 PowerPoint 裡改版面,不是在 Python 裡調座標)。
  **代價**:新模板的封面/目錄/封底必須在 pptx 裡真的有那幾頁。(§20)
...
### 8.6 大綱契約先行選版(2026-07-30)

**症狀**:舊流程把切頁、選頁型、內容壓縮混在同一次模型決策。模型會反覆選最熟悉
的通用頁型,也可能填到一半才發現來源項目數與版型下限不合；為保住版型再改造
分類/時序/因果,內容忠實與多樣性互相衝突。

**決定**:改成「來源逐字候選 → 契約先驗 → 全局選版 → 骨架 → 填槽」。
模型手產 `page_type_candidates.json`,每頁只交來源逐字 `source_excerpt`、語意 fit
與來源實際 counts；`make_skeleton.py --plan` 讀模板包 merged 契約排除不合候選,
只在同一 fit 等級間確定性降低相鄰重複/單一 template page 集中,再按 selected
counts 產骨架與 `slides.md`。renderer 不變、仍零隨機。

**界線**:多樣性永遠是語意同等候選的決勝條件,不能把並列資訊改成流程/循環來換版。
系統自行選型不得以「待補充」湊結構下限；只有使用者明確點名的缺料頁可標
`requested_by_user:true` 保留。若所有語意合適候選都不合,結論是頁型庫缺口,
不是改寫內容。R14 正反向驗 source_excerpt、容量排除、確定性序列與 counts 骨架。

### 8.7 候選廣度工具稽核化(2026-07-31)

**症狀**:8.6 上線後設計師仍回報產出雷同。盤根因:選型工具只能在模型手寫的
候選池裡挑,而模型慣性只提名最熟的兩個通用型;fit 全標 exact 給單一候選時
多樣性機制整個短路;order tie-break 讓每份 deck 都收斂到候選清單第一位。
「候選要夠廣」先前只存在於期望裡,連文件都沒寫——違反「設計不變式要工具強制」。

**決定**(plan v2,三件事全在 `make_skeleton.py --plan`):

- **整庫覆蓋審視**:plan 頂層 `not_nominated` 必須讓「提名 ∪ 有理由的未提名」
  涵蓋全部非結構全自動頁型(同族可 `字首_*`,已提名者自動略過),缺漏 exit 1
  逐一列出含結構包絡。工具驗覆蓋完整;理由真偽仍是模型責任,但會原樣寫進
  `page_type_plan.json` 的 `library_review` 供設計師稽核。
- **廣度統計 + WARN**:composition 增 `content_slides_single_candidate` 等,
  過半內容頁單一候選印「候選池過窄」。
- **同分決勝改來源片段 hash**:零隨機、同輸入必同輸出,但不同內容的 deck
  不再永遠收斂到模型慣性排第一的頁型;candidates 列序不再影響結果。

**界線**:覆蓋審視擋的是「沒看過整庫」,不判語意;fit 誠實與理由真偽仍靠
模型自律+目檢。多樣性依然只在同等 fit 內決勝(R14 b 不變)。R14 擴充
覆蓋反向測試;fixture 06 升 version 2。

### 8.8 大綱豐富訪談 enrich-outline(2026-07-31)

**症狀**:8.6/8.7 把選型端修完後,雷同的最後一個根因仍在——版型庫大多數頁型
需要特定語意結構(時序、循環、對比、層級、數據),平鋪直敘的大綱在「不捏造」
原則下只能合法映射到少數通用 info 型,選型再強也變不出來。

**決定**:不放鬆防捏造,把內容創作移到使用者仍是作者的階段。新
`engine/rules/enrich_outline_skill.md`(Knowledge 第 11 檔):產檔前的訪談
步驟,對照 `--list` 的版型結構詞彙逐項提案(結構增補標明解鎖頁型;數字與
事實只能問使用者;缺料「待補充」),唯一確認關卡=核准豐富後大綱。核准版裡
非原文逐字的行**行首標 `[補]`**,原稿先落檔 `outline_original.txt`。

**工具強制**(audit_provenance `--original`,run_pipeline 透傳):未標記行
必須逐字出自原稿(沒標記的改寫直接 FAIL);來源含 `[補]` 卻缺 `--original`
也 FAIL;報告印增補統計(幾行、幾行含數字)供設計師稽核。R15 正反向測試。

**界線**:`[補]` 行內容是否真來自使用者回覆,工具驗不了——防捏造的最後一段
仍是模型自律+人審,但所有增補集中且顯眼,洗不進「原文」。豐富非必經:
一鍵交付摘要在使用頁型偏少時提示,不設閘門、不動一鍵流程。
...
### 9.4.1 又兩個(2026-07-27,實跑產檔目檢抓到)

前五個是量測**算錯**,這兩個是量測**根本沒看**。都是拿系統跑一份真實簡報
(職場禮儀 10 頁)時,開檔目檢發現封面破版才挖出來的——**全部機器檢查
(validator、qa、fit、R12)當時都是綠的**。

6. **字級回退預設值 = 218 個框的量測全是假的。** `_first_run_size_pt` 在 run
   沒寫 `sz` 時直接回傳 `DEFAULT_FONT_PT`(18pt)。但 placeholder 的真實字級
   寫在 layout/master 的 `<a:lstStyle><a:lvlNpPr><a:defRPr sz=…>`,那裡
   python-pptx 的 `paragraph.font` **讀不到**(它對應的是 `a:pPr/a:defRPr`)。
   light 模板 59 頁裡 **218 個文字框**都靠繼承取字級,於是一律被當成 18pt。
   封面主標真值 40pt、框寬 4.21 吋:填 8 個字估算器算成 2.0 吋(實際 4.44 吋)
   → 判「裝得下」→ fit 不收緊(契約 20 字原封不動)→ PowerPoint 一開檔
   主標折兩行壓到副標與日期列。修法是沿繼承鏈查 lstStyle:自身 → layout 同 idx
   → master 同型 → master `txStyles`。重量測後 `cover.main_title` 20 → **7 字**。
7. **偵測端漏了橫向,等於那種槽位從沒量過(陷阱 11 的復發)。** `wrap="none"`
   的框行數恆等於硬換行數,所以 suspect 迴圈的 `if est["lines"] <= 1: continue`
   會把它整個跳過;橫向檢查只寫在 `_clean_cap` 內部——**候選都選不進來,
   內部檢查再對也沒用**。症狀:KPI 大數字框(1.43 吋 @66pt)填「48 小時」
   右邊爆出 0.96 吋,而 fit 印「收斂」,因為它剛好沒撞到任何**有文字**的鄰居
   (訊號②只看文字對文字)。重量測後 `kpis.value` → **2 字**(模板原文正是
   「04」「25」兩位數)、`axis_labels` → 2 字。

**新增的教訓**:①**凡是「查不到就用預設值」的量測,都會在最需要它的地方說謊**
——查不到應該是「查不到」,不是「當作 18pt」。②修一個判準時,要同時問
「這個判準負責的**候選集合**是誰挑的」;陷阱 11 在 2026-07-26 修的是 `_clean_cap`
的計算,漏了上游的挑選,於是同一個洞留了一整批頁型。③**這兩個都是實跑一份
真簡報才發現的**——golden 跑的是契約上限的合成內容,真實內容的長度分佈不一樣
(golden 的 cover 主標填滿 20 字反而更早爆,卻因為字級算錯而沒爆)。
§5.4 的「機器綠 + 人點頭,缺一不可」在這裡又應驗一次。
...
## 10.7 builtin 字樣清零(2026-07-27)——現況文字與歷史紀錄分家

§10.6 已經把「宣稱 builtin 仍在運作」的那一句從出貨包裡挖掉,程式碼也沒有那條
路徑了。剩下的是**過去式的殘影**:`page_map.md` 有整節「builtin 清零紀錄」,
三列頁型備註寫「自 builtin 遷入」,`bindings.json` 四段 `_note` 也都以它開頭,
`pack_loader` 的錯誤訊息拿「builtin 繪製器已清零」當理由。全部語意正確、全部
是歷史事實。

**判準改了:在描述「現在怎麼運作」的文字裡,過去式提到一個不存在的概念,
和現在式一樣有害。** 兩種讀者都會壞掉——模型讀 `template_light.zip` 是為了知道
現在能做什麼,看到一個模式名就會去推敲它、找它、問要不要用它;人讀 page_map
的表頭「本包無 unsupported,也已無 builtin」,第一反應是「所以支援等級有三種以上?」
真相是只有兩種。要判斷一句話該不該留,問的不是「它對不對」,而是
**「不知道這段歷史的人讀了,會不會多出一個不存在的概念」**。

於是切一條線:

- **現況文字零 builtin**——出貨的 `page_map.md`、`bindings.json`;tools.zip 的
  `pack_loader`、`pptx_toolkit`;repo 的 `ARCHITECTURE.md`、`INDEX.md`、
  各 `REGRESSION.md` 的當下說明、`regress.py` 與 `prepare_env.py` 註解。
  凡是要提「不准夾帶 Python 綁定」,一律改成陳述現行規則(「引擎不執行包內
  Python,綁定只能是 `bindings.json`」),不再用一個已廢除的模式名當理由。
- **歷史紀錄照原樣留著**——§10.5/§10.6 與 `engine/REGRESSION.md` 的變更軌跡
  是「為什麼沒有 builtin」的唯一答案,也是「推翻先例」這個決定的證據。
  把它們一起清掉,下一個人只會重新發明它。

同批修掉兩個**因為刪東西而產生的**次生缺陷:

- **§5.3 那條「新模板不得有 builtin 頁型」還寫成現行取捨**。它在 §10.5 就被
  推翻了(索引表早已註明),但沒有人回頭改本文——直接讀 §5.3 的人拿到的是一條
  過期規則。加刪除線 + 指路 §10.5/§10.6,理由段落保留(那部分仍成立且被沿用)。
- **`engine/REGRESSION.md` 指向 `page_map.md` 遷移表的連結變成斷鏈**,因為表被
  移走了。刪一段文字要順手查誰指著它——這是本批自己差點犯的錯。

例外只留兩處,都在「有人真的打出 builtin 這個字」時才會出現:`template_admin`
對 `mode == "builtin"` 的指路 hint(那正是最需要它的時刻),與 pack 裡「刪檔漏了
四份」的事故註解。

**順帶抓到的舊帳**:`gpts/DEPLOY.md` 與 `gpts/README.md` 三處把版本代號寫死成
`v2.3-20260727` / `light@2026-07-27.1`,而 instructions 早已是 v2.5 / `.3`。
那三處是**驗收步驟自己的預期值**——照著做的人會得到「版本不對 → 發版失敗」的
假紅燈。改成指向 `instructions.md` 第一行,不再複製數字(同 §9.6 的文件漂移
族類,治法也一樣:不要有第二份真相)。

產檔行為零變化,兩支 zip 因註記字串重打:light@2026-07-27.3 → .4、
instructions v2.5 → v2.6-20260727,`regress.py` 16 案例全綠。
